...
export interface IToolManifest<TInput = any, TOutput = any> {
   ...
   */
  validateInput: (input: TToolInput<TInput>) => { valid: boolean; errors?: string[] };
  /** 
   * Output schema (e.g., a Zod schema or JSON schema) for validating TToolOutput<TOutput>. 
   * For simplicity, we'll use a function type for validation initially.
   */
  validateOutput: (output: TToolOutput<TOutput>) => { valid: boolean; errors?: string[] };
...
export class ToolExecutionError extends Error {
  ...
  public input?: TToolInput<any>;

  constructor(message: string, options?: { cause?: Error; toolName?: string; input?: TToolInput<any> }) {
    super(message);
