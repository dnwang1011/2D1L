...
class CardRepository {
    ...
    async getCards(userId, filters = {}) {
        ...
        const { cardType, evolutionState, limit = 20, offset = 0, sortBy = 'updated_at', sortOrder = 'desc' } = filters;
        try {
      ...
      `;
            ...
            const cardResults = await prisma.$queryRawUnsafe(cardsQuery, ...params);
          ...
          ORDER BY score DESC
        `;
                const growthResults = await prisma.$queryRawUnsafe(growthQuery, userId, entity_id, entity_type);
    ...
    async getCardDetails(cardId, userId) {
      ...
      `;
            const cardResults = await prisma.$queryRawUnsafe(cardQuery, cardId, userId);
    ...
    async getTopGrowthCards(userId, limit = 10) {
        ...
        LIMIT $2
      `;
            const growthResults = await prisma.$queryRawUnsafe(recentGrowthQuery, userId, limit);
            const cards = [];
