...
async function shutdownWorker(signal) {
  logger.info(`[EpisodeAgentWorker] Received ${signal}. Initiating graceful shutdown...`);
  // Assuming 'worker' is the BullMQ worker instance
  if (worker && typeof worker.close === 'function') {
    await worker.close();
    logger.info('[EpisodeAgentWorker] BullMQ worker closed.');
  }
  // Add any other specific cleanup for this worker
  logger.info('[EpisodeAgentWorker] Graceful shutdown complete. Exiting.');
  process.exit(0);
}
