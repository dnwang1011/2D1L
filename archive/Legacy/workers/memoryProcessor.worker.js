...
const { initializeAIProvider } = require('../config/aiServiceInitializer');
...
async function main() {
  try {
    logger.info('[MemoryWorker] Initializing AI Service...');
    await initializeAIProvider();
    logger.info('[MemoryWorker] AI Service initialized successfully.');

    if (memoryManager.initialize) {
      logger.info('[MemoryWorker] Initializing MemoryManager...');
      await memoryManager.initialize();
      logger.info('[MemoryWorker] MemoryManager initialized.');
    }
    ...
    logger.error('[MemoryWorker] Critical error during initialization:', error);
    process.exit(1);
  }
}

main().catch(err => {
  logger.error('[MemoryWorker] Unhandled error during worker startup or operation:', err);
  process.exit(1);
}); 