...
describe('DialogueAgent E2E', () => {
  ...
  beforeAll(async () => { // Make beforeAll async
    // GOOGLE_API_KEY check remains, as it should be loaded by jest.setup.js
    if (!process.env.GOOGLE_API_KEY) {
      throw new Error('GOOGLE_API_KEY is not defined in the environment variables. Make sure .env is in the project root and jest.setup.js is working.');
    }
